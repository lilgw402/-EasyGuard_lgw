...
def inference(rank, world_size, args):
    ...
    for test_batch in prog_bar:
        ...
        input_ids = test_batch['input_ids'].to(device).unsqueeze(0)
        ...
        if not args.ouput_logits:
            ...
            print(response)
            pred = "1" if response[0][0] == "是" else "0"
            rf.write(data_id + "\t" + str(pred) + "\t" + str(gt_label) + "\t" + '\n')
        else:
            input_token_len = input_ids.shape[1]
            ...
            pred = "1" if response[0][0] == "是" else "0"
            rf.write(data_id + "\t" + str(pred) + "\t" + str(gt_label) + "\t" + generated_scores[0] + '\n')
            rf.flush()
    rf.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, required = False, default = '/mnt/bn/yangmin-priv-fashionmm/wangzhen/data/product/tupu_jinshou_test_4k.json')
    ...
    parser.add_argument("--out_path", type=str, required = False, default = 'valley/inference/infer_out/data-ys-v4-valley-product-7b-jinshou-class-lora-multi-class-old2.txt' )
