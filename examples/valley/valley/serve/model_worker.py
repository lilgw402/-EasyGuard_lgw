...
from valley.model.language_model.valley_llama import ValleyVideoLlamaForCausalLM
...
def load_model(model_path, model_name, num_gpus):
    ...
    if 'lora' in model_path:
        ...
        if 'config.json' in os.listdir(model_path):
            model_old = ValleyVideoLlamaForCausalLM.from_pretrained(model_path)
        else:
            model_old = ValleyVideoLlamaForCausalLM.from_pretrained(config.base_model_name_or_path)
        ...
        model = ValleyVideoLlamaForCausalLM.from_pretrained( model_path, torch_dtype=torch.float16 )
        print('load end')
    # for multi image
